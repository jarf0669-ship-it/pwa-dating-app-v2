...
function setAppMode(mode) {
  ...
  const clubWarning = document.getElementById('club-warning-box');
  ...
  if (mode === 'amora') {
    ...
    clubWarning.classList.add('hidden');
  } else {
    ...
    clubWarning.classList.remove('hidden');
  }
  renderGrid();
}
...
function renderChats() {
  ...
  container.innerHTML = profiles.map(p => `
    <div onclick="openMessageModal()" class="flex items-center space-x-3 bg-slate-900 p-2.5 rounded-xl cursor-pointer border border-slate-800">
      ...
      <div>
        <p class="text-xs font-bold">${p.name} <span class="text-[9px] text-pink-400 font-normal">✨ Match Mutuo</span></p>
        <p class="text-[10px] text-gray-400">Toca para enviar mensaje o toque...</p>
      </div>
    </div>
  `).join('');
}
...
function sendChatMessage() { closeMessageModal(); document.getElementById('input-chat-msg').value = ''; }
...
function nextCard() { currentProfileIndex = (currentProfileIndex + 1) % profiles.length; updateCarrusel(); }
function likeCard() { nextCard(); }

function toggleFilters(show) { document.getElementById('modal-filters').classList.toggle('hidden', !show); }
function selectFilterSex(btn) {
  document.querySelectorAll('.sex-btn').forEach(b => { b.className = "p-2 rounded-xl bg-slate-800 text-gray-300 sex-btn"; });
  btn.className = "p-2 rounded-xl bg-pink-600 font-bold text-white sex-btn";
}

function openAuthModal(type) {
  const title = document.getElementById('auth-title');
  const passBox = document.getElementById('auth-pass-box');
  if (type === 'signin') { title.innerText = "Iniciar Sesión (Sign In)"; passBox.style.display = 'block'; }
  else if (type === 'signup') { title.innerText = "Crear Nuevo Perfil"; passBox.style.display = 'block'; }
  else if (type === 'recovery') { title.innerText = "Recuperar Contraseña"; passBox.style.display = 'none'; }
